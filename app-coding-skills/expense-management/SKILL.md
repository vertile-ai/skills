---
name: app-coding-skills-expense-management
description: >-
  Used when building expense management software on the web. Expense management workflow for submission, policy checks, approvals, reimbursement, and audit evidence.
---

# Expense Management playbook

## Purpose / category boundary

Expense management workflow for submission, policy checks, approvals, reimbursement, and audit evidence.

## When to use

- You need a dedicated product workflow for **expense management**, not a generic SaaS shell.
...
- Double-entry-aware ledger tables
- Idempotent payment webhooks
- Async invoice and payout jobs
...
Order/subscription events -> billing ledger mutation -> invoice settlement -> reconciliation.

Recommended entity backbone:

- `tenant`
- `expense_management_record`
- `workflow_state`
- `activity_event`
- `integration_event`
...
- Use immutable ledger entries; compute balances from journal lines, not mutable totals.
...
- Never couple payment provider webhook latency to user-facing write transactions.
